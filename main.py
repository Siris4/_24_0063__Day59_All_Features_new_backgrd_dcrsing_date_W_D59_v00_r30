    ...
    static_posts = [
        {
            ...
            "date": dt.datetime.strptime("2023-09-24", "%Y-%m-%d")
        },
        {
            "title": "I believe every human has a finite number of heartbeats. I don't intend to waste any of mine.",
            "author": "Start Bootstrap",
            "date": dt.datetime.strptime("2023-09-18", "%Y-%m-%d")
        },
        {
            "title": "Science has not yet mastered prophecy",
            "subtitle": "We predict too much for the next year and yet far too little for the next ten.",
            "author": "Start Bootstrap",
            "date": dt.datetime.strptime("2023-08-24", "%Y-%m-%d")
        },
        {
            "title": "Failure is not an option",
            "subtitle": "Many say exploration is part of our destiny, but it’s actually our duty to future generations.",
            "author": "Start Bootstrap",
            "date": dt.datetime.strptime("2023-07-08", "%Y-%m-%d")
        }
    ]
    try:
        ...
        for post in api_posts:
            ...
            try:
                post['date'] = dt.datetime.strptime(post.get('date', ''), "%Y-%m-%d")
            except ValueError:
                post['date'] = dt.datetime.now()  # Default to today's date if parsing fails
